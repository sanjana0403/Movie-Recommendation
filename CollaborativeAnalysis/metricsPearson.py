...
for j in range(len(testmtrx)):
	...
	pearsonSim = zeros(trainUsers)

	for row in range(len(mtrx)):
		pearsonSim[row] = st.pearsonr(user, mtrx[row])[0]

	indexpearson = argpartition(pearsonSim,1) 
	similarpearson = indexpearson[:1]

	count = 0
	sums = 0
	for i in range(len(testmtrx[j])):
		if (testmtrx[j][i]!=0) and (mtrx[similarpearson[0]][i]!=0):
			#print "in if"
			count = count+1
			sums = sums+abs(testmtrx[j][i]-mtrx[similarpearson[0]][i])
			# print sums	
...
print "Net mean absolute error ",ans

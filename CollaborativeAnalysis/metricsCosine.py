...
for j in range(len(testmtrx)):
	...
	cosineSim = zeros(trainUsers)

	for row in range(len(mtrx)):
		cosineSim[row] = sp.cosine(user, mtrx[row])

	indexcosine = argpartition(cosineSim,1) 
	similarcos = indexcosine[:1]

	count = 0
	sums = 0
	for i in range(len(testmtrx[j])):
		if (testmtrx[j][i]!=0) and (mtrx[similarcos[0]][i]!=0):
			#print "in if"
			count = count+1
			sums = sums+abs(testmtrx[j][i]-mtrx[similarcos[0]][i])
			# print sums	

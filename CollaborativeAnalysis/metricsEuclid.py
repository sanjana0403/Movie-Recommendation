...
for j in range(len(testmtrx)):
	...
	euclid = zeros(trainUsers)

	for row in range(len(mtrx)):
		euclid[row] = sp.euclidean(user,mtrx[row])

	indexeuclid = argpartition(euclid,1)
	similareuc = indexeuclid[:1]

	count = 0
	sums = 0
	for i in range(len(testmtrx[j])):
		if (testmtrx[j][i]!=0) and (mtrx[similareuc[0]][i]!=0):
			#print "in if"
			count = count+1
			sums = sums+abs(testmtrx[j][i]-mtrx[similareuc[0]][i])
			# print sums	
